...
export function registerCacheCommand(program: Command): void {
  ...
  cache
    ...
    .action((opts) => {
      ...
      if (!meta) {
        ...
        console.log(chalk.gray('Run "linked-agent list" to populate the cache.'))
        return
      }
  ...
  cache
    .command('clear')
    .description('Delete the local cache directory')
    .action(() => {
      const parentOpts = cache.opts<{ cacheDir?: string }>()
      const cacheDir = getCacheDir(parentOpts.cacheDir)
      clearCache(cacheDir)
